 ...
 //     const resMint = await axios.get( 'http://localhost:8080/Top100?Action=Top100Polygon', {
     ...
     // console.log(resMint.data[0]);
      ...
      const rows = accountMetaData.map((row,index) => 
          ...
          <li className='row' ><span className='rank' ><div className='contentDiv'>{row.rank == 1 ? <div className="imgspan">&nbsp;</div>: row.rank == 2 ? <div className="imgspan2">&nbsp;</div> : row.rank == 3 ? <div className="imgspan3">&nbsp;</div> : row.rank}</div></span><span className='link' ><div className='contentDiv'><a target='_blank' href={'https://opensea.io/' +row.link}>{row.link.replace('https://opensea.io/','').substring(0,6)}...{row.link.replace('https://opensea.io/','').substring(37)}</a></div></span><span className='buy' ><div className='contentDiv'>{row.buys}</div></span><span className='avgprice' ><div className='contentDiv'>$ {( row.avgprice * 1 ).toFixed(2) }</div></span><span className='sold' ><div className='contentDiv'>{ row.sold }</div></span><span className='profit' ><div className='contentDiv'>$ { (numberWithCommas( (row.profit * 1 ).toFixed(2) ) ) }</div></span><span className='roi' ><div className='contentDiv'>{(row.roi*1).toFixed(0)} %</div></span><span className='genesis' ><div className='contentDiv'>{ row.genesis }</div></span><span className='follow' ><div className='contentDiv'><a target='_blank' href={'https://opensea.io/' + row.link}><img className='followimg' src={ follow }/></a></div></span></li>
